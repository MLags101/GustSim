...
def geometry_provenance(run):
    """Whether the loads were computed on the imported CAD or on an approximation of it.

    A wrapped revision must never reach a report or a data file without saying so.
    """
    from . import db
    try:
        meta = db.geometry(run['spec']['geometry_id'])
    except (KeyError, TypeError):
        return {'fidelity': 'unknown'}
    fidelity = meta.get('geometry_fidelity') or 'imported'
    record = {'fidelity': fidelity, 'geometry_id': meta.get('id'), 'sha256': meta.get('sha256')}
    if fidelity == 'wrapped':
        record['wrap'] = meta.get('wrap')
        record['caveat'] = ('Loads were computed on a shrink-wrapped approximation, not on the '
                            'imported CAD. Features finer than the wrap resolution are absent.')
    return record


def manifest(run):
    return {'run_id':run['id'],'created':run['created'],'status':run['status'],'configuration':run['spec'],
            'openfoam_version':config.FOAM_VERSION,'quality':run['result'],'geometry':geometry_provenance(run),
...
def report_html(run):
    ...
    provenance=geometry_provenance(run)
    banner=''
    if provenance.get('fidelity')=='wrapped':
        detail=(provenance.get('wrap') or {}).get('detail','')
        banner=(f'<p class="caveat"><strong>Approximated geometry.</strong> {esc(provenance["caveat"])} '
                f'{esc(detail)}</p>')
    return f'''<!doctype html><html lang="en"><meta charset="utf-8"><title>GustSim report</title>
<style>body{{font:16px/1.6 system-ui;max-width:950px;margin:50px auto;padding:20px;color:#182c38}}h1{{font-weight:500}}table{{border-collapse:collapse;width:100%}}td{{padding:8px;border-bottom:1px solid #ccd6df}}pre{{white-space:pre-wrap;background:#f1f5f8;padding:20px}}strong{{color:#205b61}}.caveat{{background:#fdf3e2;border-left:4px solid #d08b2c;padding:12px 16px}}</style>
...
Experimental template validation: pending.</p>{banner}<h2>Quality findings</h2><ul>{findings}</ul><h2>Engineering quantities</h2><table>{metrics}</table>
