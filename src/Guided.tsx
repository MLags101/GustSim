...
export function GuidedSetup({geometry,spec,setSpec,selected,setSelected,act,onFlowPreview}:any){
  ...
  const [kind,setKind]=useState(spec.use_case?.kind||'aerodynamics'),[fluid,setFluid]=useState(spec.fluid.name==='water'?'water':'air'),[speed,setSpeed]=useState(spec.flow.speed),[direction,setDirection]=useState<number[]>(spec.references.drag_axis.map((n:number)=>-n)),[lift,setLift]=useState<number[]>(spec.references.lift_axis);
  const [inlet,setInlet]=useState(''),[outlet,setOutlet]=useState(''),[driving,setDriving]=useState('flow'),[flow,setFlow]=useState(.01),[pin,setPin]=useState(100),[pout,setPout]=useState(0),[turbulence,setTurbulence]=useState(spec.flow.turbulence);
  const [origin,setOrigin]=useState<number[]>(spec.rotation.enabled?spec.rotation.origin:geometry.bounds[0].map((n:number,i:number)=>(n+geometry.bounds[1][i])/2)),[axis,setAxis]=useState<number[]>(spec.rotation.axis),[rpm,setRpm]=useState(spec.rotation.enabled?spec.rotation.rpm:2000),[preview,setPreview]=useState<any>(),[confirmed,setConfirmed]=useState(false),[refs,setRefs]=useState(false);
