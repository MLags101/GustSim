    ...
    <div className="table-scroll"><table className="compare-table">
      ...
      <tbody>{rows.map(([name,key,unit])=>{
        ...
        return <tr key={key}>
          ...
          <td>{fmt(first)}</td>
          <td>{fmt(second)}</td>
          <td className={delta===null?'':delta>0?'delta-up':'delta-down'}>{delta===null?'—':`${delta>0?'+':''}${delta.toFixed(1)}%`}</td>
        </tr>;
      })}</tbody>
    </table></div>
